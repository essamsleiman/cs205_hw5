...
__global__ void compute_logits_coalesced(double *input_features, double *weights, double *biases, double *logits, int num_samples, int feature_dim, int num_classes)
{
    ...
    if (sample_idx < num_samples && class_idx < num_classes)
    {
        ...
        {
            linear_combination += input_features[feature_offset + feature_idx] * weights[feature_idx + class_idx * feature_dim];
        }
                   ...
                   double* gpuBias)
{
    ...
    // Transpose kernel matrix and copy to GPU for coalesced access
    ...
    for (int i = 0; i < kernelRowCount; i++)
    {
        ...
        {
            // Transpose the matrix here by swapping indices
            gpuKernel[k * kernelRowCount + i] = inputKernel[i][k];
        }
    }
...
int main()
{
    ...
    for (int iter = 0; iter < 10; iter++)
    {
        ...
        // cuda kernel
        compute_logits_coalesced<<<gridDim, blockDim>>>(d_features, d_kernel, d_bias, d_result, feature.size(), kernel.size(), bias->size());
        ...
        cout << "Iteration2 " << iter << " Time taken: " << chrono::duration_cast<chrono::milliseconds>(end - start).count() << " ms" << endl;
